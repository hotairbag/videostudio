...
const Production: React.FC<ProductionProps> = ({
  ...
  const clipDuration = videoModel === 'seedance-1.5'
    ? (seedanceDuration ?? 4)
    : CLIP_DURATION_BY_MODEL[videoModel];
  ...
  const parsedCharacterRefs: CharacterRef[] = React.useMemo(() => {
    ...
    try {
      return JSON.parse(characterRefs) as CharacterRef[];
    } catch {
      return [];
    }
  }, [characterRefs]);
  ...
  const handleExport = async (includeMusic: boolean = true) => {
          ...
          {hasEnoughToWatch && (
            <div className="flex gap-2">
              {isExporting ? (
                <button
                  disabled
                  className="px-6 py-3 rounded-lg font-bold shadow-lg bg-neutral-700 text-neutral-300 cursor-wait flex items-center gap-2"
                >
                  <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                ...
                <>
                  <button
                    ...
                    className="px-4 py-3 rounded-lg font-bold shadow-lg transition-all flex items-center gap-2 bg-white text-black hover:bg-neutral-200"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-3 rounded-lg font-bold shadow-lg transition-all flex items-center gap-2 bg-neutral-700 text-white hover:bg-neutral-600"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                    No Music
                  </button>
                ...
                <button
                  onClick={() => handleExport(false)}
                  className="px-6 py-3 rounded-lg font-bold shadow-lg transition-all flex items-center gap-2 bg-white text-black hover:bg-neutral-200"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                  Download
                </button>
      ...
      {originalPrompt && (
        <div className="bg-neutral-800/50 rounded-xl border border-neutral-700 overflow-hidden">
          <button
            ...
            className="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-neutral-800/80 transition-colors"
          >
            <span className="text-sm font-medium text-neutral-300">Project Overview</span>
            <svg
              className={`w-4 h-4 text-neutral-400 transition-transform ${showProjectOverview ? 'rotate-180' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              ...
              <div className="pt-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xs font-medium text-neutral-500 mb-1 uppercase tracking-wide">Original Prompt</h3>
                    <p className="text-neutral-200 text-sm whitespace-pre-wrap break-words">{originalPrompt}</p>
                  </div>
                  <button
                ...
                {parsedCharacterRefs.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-neutral-700">
                    <h3 className="text-xs font-medium text-neutral-500 mb-3 uppercase tracking-wide">Character References</h3>
                    <div className="flex flex-wrap gap-4">
                      {parsedCharacterRefs.map((char, idx) => (

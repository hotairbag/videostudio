 ...
 * - Use structured sections (genre, instrumentation, production, mood)
 * - Use "and" instead of commas for essential elements (commas = optional)
 * - End sections with periods for clear parsing
 * - Match the actual scene content - casual scenes get casual music, epic scenes get epic music
 * - Be specific to escape the "pop gravity well"
 */
function buildSunoPrompt(title: string, style: string, theme?: string): string {
  ...
  // Build genre section - match the actual content type
  let genre: string;
  ...
  if (isCasual) {
    ...
    // Travel content - inspiring, adventurous, world-influenced
    ...
    // Lifestyle/fashion - modern, trendy, polished pop
    ...
    // Sports/fitness - energetic, driving, motivational
    ...
    // Nature/wildlife - ambient, atmospheric, organic
    ...
    // Retro content - period-appropriate style
    ...
    // Default fallback - neutral background music
  ...
  // Construct the structured prompt using Suno's preferred format
  const promptParts = [
    `genre: "${genre}".`,
...
/**
 * Build the style field for Suno API - using "and" connector per the guide
 * Match the style to the actual scene content
 */
function buildSunoStyle(title: string, style: string, theme?: string): string {
  ...
  // Connect with " and " per Suno guide (commas signal optional elements)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Build structured Suno prompt using advanced techniques from the guide
    ...
    // Generate music using Suno V5 via kie.ai
    // Using customMode with instrumental only (no vocals/dialogue)
    const generateRes = await fetch(`${BASE_URL}/generate`, {
      ...
      body: JSON.stringify({
        ...
        callBackUrl: 'https://example.com/callback' // Placeholder, we poll instead
      })
    });
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      if (statusRes?.ok) {
        ...
        if (taskData) {
          ...
          // Status might be TEXT_SUCCESS, PROCESSING, etc - continue polling
        }
      }
    }
